...
In the LASSO coefficients, the area, kernel_length, kernel_width, asymmetry, and 
...
method. The optimal K was 9, and the associated accuracy was 94.16%.
...
QDA was able to achieve an accuracy of 97.53% in the cross validation, which is 
...
With the RBF kernel, the accuracy on cross validation was 96.25%, which is only a little 
smaller than QDA.
...
KNN was able to achieve an accuracy of 92.45 on the test set, which was lower than 
...
The model performed well, with an accuracy of 94.34%, which is lower than 
the accuracy in cross validation. Furthermore, as shown by the confusion matrix, 
3 mistakes were made in the test set.
...
the confusion matrix, SVM only made 1 mistake, and it is similar to one of the QDA 
errors.
...
dataset. In both cases, they outperformed KNN. While QDA performed the best on CV, 
SVM was able to achieve a higher accuracy in the test set. Regardless, they are both 
