...
on cross validation, with an accuracy of 95.6%. They also performed identically 
on the test set with an accuracy of 98.11%, but the error in classification is different.
...
going to be used for the other methods, since it normalizes the data, leads to 
dimension reduction, and removes correlation.
...
In the LASSO coefficients, the area, kernel_length, asymmetry, and grove_length seems 
to have notable weight. Area, compactness, and grove length are involved in different 
classes. However, from the PCA analysis, the area is pretty correlated with the 
kernel_length, and their plot shows similar separation. Furthermore, the weight of 
kernel_length is greater than area. Asymmetry only appears for one class, and has 
a small weight. Furthermore, the plot of the types are close together. Finally, 
The rest of the variables are pretty correlated with each other. Thus, the final 
set of variables for QDA will be kernel_length, compactness, grove_length.
...
had the best pairwise plot for separation. But, PC4 and 5 are relevant in the coefficients 
for 2 classes, and their weights are large for type 0 wheat. So, they are also considered. 
Thus, the final set of predictors for PCA transformed data are PC1, PC2, PC4, and PC5.
...
method. The optimal K was 5, and the associated accuracy was 93.11%.

```{r}
set.seed(97) 
model.knn <- train(type~., data = adopted_train[, c(1, 2, 4, 8)], method = "knn", trControl = train.control)
...
QDA was able to achieve an accuracy of 95.62% in the cross validation, which is 
higher than the KNN result.

```{r}
set.seed(97) 
model.qda <- train(type~., data = train[, c(3, 4, 7, 8)], method = "qda", trControl = train.control)
...
With the RBF kernel, the accuracy on cross validation was 95.58%, which is similar 
to QDA.
...
KNN was able to achieve an accuracy of 94.34 on the test set, which was higher 
...
98.11%, which is also greater than the accuracy in cross validation. Based on  
the confusion matrix, SVM only made 1 mistake, but it's in a different category 
than QDA.
...
dataset. The two method performed similarly in both CV and the test set. In both cases, 
the performance was better than KNN. The different error on the test set suggests that 
the decision boundary may be different despite similar performance. Regardless, QDA can 
