...
of wheat from the transformed data, with KNN as a baseline. The QDA achieved the
best performance on both cross validation and the test set, with an accuracy of 
0.969 and 0.981 respectively.
...
Since the data are clearly not on the same unit, and most of the techniques are 
benefited if the data are normalized to standard units, the original data was 
scaled to standard units by their mean and standard deviation.
...
seed_pcs_cluster_renamed$cluster[seed_pcs_cluster$cluster == 2] <- 1
...
listed below. The clusters and types have similar mean and standard 
deviation, which indicates that the distribution of the points on the partitions 
created by KMeans is similar to the distribution of the points by type.
...
The final coefficients are shown below. In addition to PC1, PC2, PC4, PC5 discussed 
before with respect to separation, PC6 had significant weight for type 2 wheats. 
But, all other PCs have either zero weights or small weights. Thus, the PC1, PC2, 
PC5, PC5, and PC6 will be used for prediction.
...
method. The optimal K was 7, and the associated accuracy was 93.02%.

```{r}
model.knn <- train(type~., data = adopted_train[, c(1, 2, 4, 5, 6, 8)], method = "knn", trControl = train.control)
...
In this case, the elliptical shape of the points and the success of KMean suggests 
that the distribution may be approximated by a multivariate gaussian distribution. 
Furthermore, the standard deviation of the first two PCs are clearly different, which 
implies that QDA would be more fitting than LDA, which assumes the same variance 
across classes.

QDA was able to achieve an accuracy of 94.25% in the cross validation, which is 
higher than the KNN result.

```{r}
model.qda <- train(type~., data = adopted_train[, c(1, 2, 4, 5, 6, 8)], method = "qda", trControl = train.control)
...
With the RBF kernel, the accuracy on cross validation was 97.46%.

```{r}
tunegrid_rbf <- expand.grid(C=c(seq(0.01, 2, length = 20), 10), sigma=c(seq(0.01, 2, length = 20), 10))
# Train the model
model.rbfsvm <- train(type~., data = adopted_train[, c(1, 2, 4, 5, 6, 8)], method = "svmRadial",
...
In both cases, the models clearly out-performed KNN on cross validation. Furthermore, 
both QDA and SVM had similar performance on cross validation.

## KNN on Testing Set

KNN was able to achieve an accuracy of 96.23 on the test set, which was higher 
than the cross validation result.
...
The model performed pretty well, with an accuracy of 98.11%, which is higher than 
the accuracy in cross validation. Furthermore, as shown by the confusion matrix, 
only 1 mistake was made. Compared to KNN, it made 1 less mistake in the same category.
...
98.11%, which is roughly the same as the accuracy in cross validation. Based on, 
the confusion matrix, SVM only made 1 mistake similar to the one made by QDA.
...
dataset. SVM performed better in cross validation, but both performed equally on 
the test set. In both cases, the performance was better than KNN.
However, despite not performing as well as SVM on CV, QDA can be more useful than SVM, 
since it provides a complete distribution over the classes of wheat. Thus, it can 
be a good characterization of the wheat types.
