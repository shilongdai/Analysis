...
- parimenter: the perimeter of the wheat kernel
...
the plot, grove_length, kernel_length, parimenter, area, and kernel_width are 
...
listed below. The clusters and types have almost identical mean and standard 
...
method. The optimal K was 9, and the associated accuracy was 92.25%.
...
QDA was able to achieve an accuracy of 96.87% in the cross validation, which is 
...
With the RBF kernel, the accuracy on cross validation was 96.75%.
...
96.23%, which is roughly the same as the accuracy in cross validation. Based on, 
the confusion matrix, SVM only made 2 mistakes, with one being similar to the QDA. 
...
dataset. On both the cross validation as well as test set, QDA performed slightly better 
than SVM. Additionally, QDA can be more useful than SVM, since it provides a 
complete distribution over the classes of wheat. Thus, it can be a good characterization 
of the wheat types. Finally, the success of QDA suggests that the Gaussian EM 
algorithm may also achieve a good clustering of the data points.
