...
rent.logplot <- ggplot(apartments_raw_small, aes(x=log(rent))) + 
  geom_histogram(bins=bw_rent, fill="#69b3a2", 
  color="#e9ecef", alpha=0.9)
grid.arrange(rent.plot, rent.logplot, top="Distribution of Rent")
```
...
sqft.plog <- ggplot(apartments_raw_small, aes(x=log(sqft))) + 
  geom_histogram(bins=bw_sqft, fill="#69b3a2", 
  color="#e9ecef", alpha=0.9)
grid.arrange(sqft.plot, sqft.plog, top = "Distribution of Sqft")
```
...
bed_plot <- plot_as_percentage(apartments_raw_small, "beds") + xlab("") + theme_bw()
baths_plot <- plot_as_percentage(apartments_raw_small, "baths") + xlab("") + theme_bw()

grid.arrange(bed_plot, baths_plot, ncol=2, top = "Distribution of Beds and Baths")
```
...
shop.sqft.plt <- ggplot(apartments_raw_small, 
                        ...
                        geom_abline(intercept = coefs.sqft.shop[1], 
                                    ...
                                    color="red", alpha=0.5)
grid.arrange(shop.sqft.plt, shop.rent.plt)
```
...
ggplot(apartments_raw_small, aes(x=lng, y=lat)) + geom_bin2d(bins = 70) +
  ...
  ggtitle("Apartment Distributed Across Chicago")
```
...
create_score <- function(og, train_idx, score.pca=NULL) {
  df <- data.frame(og)
  df[, c("PC1", "PC2", "PC3", "PC4")] <- 0
  if(is.null(score.pca)) {
  ...
  score.pca.train <- predict(score.pca, og[train_idx, ])
  df[train_idx, c("PC1", "PC2", "PC3", "PC4")] <- score.pca.train
  df[-train_idx, c("PC1", "PC2", "PC3", "PC4")] <- score.pca.test
  df <- df[, -which(names(df) %in% 
              c("walk.score", "bike.score", "sound.score", "transit.score"))]
  return(list("data"=df, "pca"=score.pca))
}
...
apartment_sqft <- create_score(apartment_sqft, sqft_train_ind)[["data"]]
...
sqft_train_export <- data.frame(apartment_sqft_scaled[sqft_train_ind, ])
sqft_test_export <- data.frame(apartment_sqft_scaled[-sqft_train_ind, ])
write.csv(sqft_train_export, "sqft_train.csv", row.names = FALSE)
write.csv(sqft_test_export, "sqft_test.csv", row.names = FALSE)
...
apartment.score <- create_score(apartments_raw_small, rep(T, nrow(apartments_raw_small)))
apartment.composite <- apartment.score[["data"]]
apartment.sqft.pca <- apartment.score[["pca"]]
sqft.fill.scaled <- scale_apartment(apartment.composite[, 
                  ...
                  apartment.composite)
write.csv(sqft.fill.scaled, "fill_sqft.csv", row.names = FALSE)
...
full.sqft.coef <- coef(full.lasso.sqft, s = 'lambda.min')
selected.sqft <- rownames(full.sqft.coef)[full.sqft.coef[,1]!= 0][-1]
sqft.train <- sqft.train[, selected.sqft]
full.lasso.sqft <- glmnet(sqft.train, sqft.target, 
                          lambda=cv.lasso.sqft$lambda.min)

save(full.lasso.sqft, file="sqft.mod.RData")
save(selected.sqft, file="sqft.coefs.RData")
save(apartment.sqft.pca, file="sqft.pca.RData")
```
...
```{r message=FALSE, error=FALSE, warning=FALSE}
write.csv(apartments_raw_small, "full.csv", row.names = FALSE)
...
apartment_rent <- create_score(apartment_rent, rent_train_ind)[["data"]]
...
rent_train_export <- data.frame(apartment_rent.scaled[rent_train_ind, ])
rent_test_export <- data.frame(apartment_rent.scaled[-rent_train_ind, ])
...
apartment_rent <- apartments_raw_small[, -which(names(apartments_raw_small) %in% 
                  c("name", "address", "state", "city"))]
apartment.score <- create_score(apartment_rent, rep(T, nrow(apartment_rent)))
apartment_rent <- apartment.score[["data"]]
apartment.rent.pca <- apartment.score[["pca"]]
...
full.rent.coef <- coef(full_lasso_mod, s = 'lambda.min')
selected.rent <- rownames(full.rent.coef)[full.rent.coef[,1]!= 0][-1]
lasso_rent.full <- lasso_rent.full[, selected.rent]
full_lasso_mod <- glmnet(lasso_rent.full, lasso_target.full, 
                          lambda=cv.lasso.rent$lambda.min)

save(full_lasso_mod, file="rent.mod.RData")
save(selected.rent, file="rent.coefs.RData")
save(apartment.rent.pca, file="rent.pca.RData")
