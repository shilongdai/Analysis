...
```{r}
apartment_full <- read.csv("full_sqft.csv") %>% filter(beds <= 3, baths <= (beds + 0.5))
...
In order to speed up the computation, the selection process is ran in parallel.
...
```{r}
best_lasso <- select_penal_param(apartment_train, 1, 101, 0)
...
```{r}
log.log.mod <- lm(log(rent) ~ log(sqft) + zip + baths + beds + sqft.regressed + has.covered + has.garage + has.lot + has.street, data = apartment_merged)
untrans.gam <- gam(rent ~ s(sqft, k = 2001, m = 2) + zip + baths + beds + sqft.regressed + has.covered + has.garage + has.lot + has.street, data = apartment_merged, method = "REML")
gam.mod.log <- gam(log(rent) ~ s(log(sqft), k = 2001, m = 2) + zip + baths + beds + sqft.regressed + has.covered + has.garage + has.lot + has.street, data = apartment_merged, method = "REML")

perf_info_metrics <- data.frame(
  Method = c("Log-Log-LM", "Plain-GAM", "Log-Log-GAM"),
  AIC = c(AIC(log.log.mod), AIC(untrans.gam), AIC(gam.mod.log)),
  BIC = c(BIC(log.log.mod), BIC(untrans.gam), BIC(gam.mod.log))
)
```
